...
// a protocol-relative path (`//evil.com/x`) â€” the same hole the Deno worker had.
//
...
describe("vercel relay reuses the SSRF-safe resolveRelayTarget guard", () => {
  it("accepts a legitimate absolute path", () => {
    ...
    if (r.ok) assert.equal(r.url, "http://localhost:20128/v1/foo");
  });
...
describe("buildRelayFunction â€” generated Vercel worker has no string-concat SSRF hole", () => {
