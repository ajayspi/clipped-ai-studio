...
// â”€â”€ model-combo-mappings routes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ sync token hashing (src/lib/sync/tokens.ts) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
test("hashSyncToken is deterministic â€” same input always produces same output", () => {
  const token = syncTokens.generatePlaintextSyncToken();
  assert.equal(
...
// â”€â”€ sanitizeUpstreamDetails â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

test("sanitizeUpstreamDetails â€” basic pass-through for safe fields", async () => {
...
test("sanitizeUpstreamDetails â€” sanitizes string values (absolute path)", async () => {
  const { sanitizeUpstreamDetails } = await import("../../open-sse/utils/error.ts");
...
test("sanitizeUpstreamDetails â€” removes blocked keys (stack, apiKey)", async () => {
  const { sanitizeUpstreamDetails } = await import("../../open-sse/utils/error.ts");
  const input = {
    error: { message: "oops" },
    stack: "Error\n    at foo.ts:1",
...
test("sanitizeUpstreamDetails â€” depth cap replaces nested value at depth > 4", async () => {
  const { sanitizeUpstreamDetails } = await import("../../open-sse/utils/error.ts");
  // Build depth-6 nesting: a.b.c.d.e.f = "leaf"
  const input = { a: { b: { c: { d: { e: { f: "leaf" } } } } } };
  const out = sanitizeUpstreamDetails(input) as any;
  // depth 0:a, 1:b, 2:c, 3:d, 4:e â†’ e is at depth 4, f would be depth 5 â†’ truncated
  assert.equal(out.a.b.c.d.e, "[truncated]");
});

// â”€â”€ buildErrorBody with upstreamDetails â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

test("buildErrorBody â€” without upstream details omits upstream_details field", async () => {
  const { buildErrorBody } = await import("../../open-sse/utils/error.ts");
...
test("buildErrorBody â€” with safe upstream details embeds upstream_details", async () => {
  const { buildErrorBody } = await import("../../open-sse/utils/error.ts");
  const body = buildErrorBody(400, "bad request", {
...
test("buildErrorBody â€” upstream details with stack key are stripped", async () => {
  const { buildErrorBody } = await import("../../open-sse/utils/error.ts");
...
// â”€â”€ createErrorResult with upstreamDetails â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

test("createErrorResult â€” response body includes upstream_details when provided", async () => {
...
test("createErrorResult â€” response body excludes upstream_details when not provided", async () => {
  const { createErrorResult } = await import("../../open-sse/utils/error.ts");
...
test("createErrorResult â€” exposes error code/type on the result object", async () => {
  const { createErrorResult } = await import("../../open-sse/utils/error.ts");
...
// â”€â”€ createErrorResult.rawMessage (#7360) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
// `error` is sanitized to its first line (sanitizeErrorMessage) for the
// client-facing response body â€” correct per Hard Rule #12. But internal
...
// the returned object only â€” it must never leak into the HTTP response body.

test("createErrorResult â€” rawMessage preserves the full multi-line message untruncated", async () => {
  const { createErrorResult } = await import("../../open-sse/utils/error.ts");
  const fullMessage =
...
test("createErrorResult â€” rawMessage never appears in the serialized response body", async () => {
  const { createErrorResult } = await import("../../open-sse/utils/error.ts");
  const fullMessage =
    "You exceeded your current quota, please check your plan and billing details.\n" +
    "* Quota exceeded for metric: generativelanguage.googleapis.com/generate_content_free_tier_input_token_count, limit: 16000, model: gemma-4-31b";
...
// â”€â”€ existing tests continue â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
