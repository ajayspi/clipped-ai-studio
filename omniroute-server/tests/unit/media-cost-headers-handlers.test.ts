...
// Cost may legitimately be 0 (free / unpriced modality) â€” formatOmniRouteCost
...
test("rerank NVIDIA-format success Response reflects synthesized search unit in cost header", async () => {
  ...
  saveSyncedPricing({
    nvidia: {
      ...
      // (strips the "nvidia/" prefix) â†’ this key resolves on the second try.
  ...
  globalThis.fetch = (async (url: unknown) => {
    ...
    if (stringUrl === "https://integrate.api.nvidia.com/v1/ranking") {
      // NVIDIA shape â€” note: NO `meta` block at all.
      return new Response(
        JSON.stringify({
          rankings: [
  ...
  // 1 synthesized search unit Ã— $0.002 = $0.002. With the OLD `data?.metaâ€¦`
...
test("moderation handler success Response carries cost telemetry headers (cost 0)", async () => {
  globalThis.fetch = (async (url: unknown) => {
    const stringUrl = String(url);
    if (stringUrl === "http://localhost:20128/v1/moderations") {
      return new Response(
        JSON.stringify({
          id: "modr-1",
  ...
  assert.equal(
    response.headers.get(OMNIROUTE_RESPONSE_HEADERS.responseCost),
    "0.0000000000",
    "moderation is free â†’ cost must be exactly 0"
  );
});
...
test("v1 audio transcription success Response carries cost telemetry headers (cost 0)", async () => {
  ...
  assert.equal(
    response.headers.get(OMNIROUTE_RESPONSE_HEADERS.responseCost),
    "0.0000000000",
    "transcription duration unavailable â†’ cost must be 0"
  );
});
