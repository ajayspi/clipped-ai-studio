...
// (isRetryableProxyTarget, isSecurityBlockError). These were a behavior-preserving move â€” no logic
// change â€” so the locks here are: public surface stays intact, the new modules expose the helpers,
...
test("headers: buildBearerHeaders sets Authorization; builders vary the scheme", () => {
  ...
  assert.equal(headers.buildBearerHeaders("").Authorization, undefined); // empty key â†’ no auth header
});
...
test("transport: isRetryableProxyTarget fails closed on bad URL and blocks private hosts", () => {
  ...
  assert.equal(transport.isRetryableProxyTarget("http://localhost:20128/v1"), true);
});
