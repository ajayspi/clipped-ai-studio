...
// mix, so it normalizes to a 5xx â€” the `fallbackOnlyOnQuotaExhaustion` STOP
// decision itself is untouched (it is driven by internal quota-observation
// tracking, not by re-reading the final HTTP status â€” asserted below via
...
test("protected parent combo-ref stops after normal child quality rejection then quota exhaustion", async () => {
  ...
  assert.deepEqual(
    ...
    "the parent must still STOP at the child (not fall back to paid/backup) â€” the " +
