...
// GHSA-4f49-hj64-448x â€” a persisted, caller-supplied providerSpecificData.baseUrl
...
// providers (vLLM, LM Studio, Ollama, â€¦) stay exempt so loopback/LAN keeps working.
...
test("BaseExecutor allows a public upstream URL for a non-local provider", () => {
  const guard = guardOf("openai");
  assert.doesNotThrow(() => guard("http://localhost:20128/v1/chat/completions"));
});
