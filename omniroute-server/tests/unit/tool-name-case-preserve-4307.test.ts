/**
 * #4307 â€” Tool name case changes (`read` -> `Read`) leaks to the client.
 *
 ...
 * sees an empty map and the cloaked `Read` streams verbatim to the client â€” the
...
class ClaudeLikeExecutor extends BaseExecutor {
  constructor() {
    super("claude", { baseUrls: ["http://localhost:20128/v1/messages"] });
  }
...
test("#4307 execute() preserves the tool-name cloak map (read->Read reverse) on the returned body", async () => {
  ...
  const result = await executor.execute({
    ...
    body: {
      ...
      // A tool literally named `read` (lower-case) â€” must round-trip unchanged
      // back to the client.
      tools: [{ name: "read", description: "read a file", input_schema: { type: "object" } }],
    },
    stream: false,
    // OAuth token (sk-ant-oatâ€¦) with NO apiKey => hasClaudeOAuthToken => cloak fires.
    credentials: { accessToken: "sk-ant-oat-test-token-4307" },
  });

  // Precondition: the cloak actually fired â€” the body sent UPSTREAM carries the
