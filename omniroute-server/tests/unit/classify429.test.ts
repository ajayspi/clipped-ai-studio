...
test("classify429: 429 with quota keyword in string body returns 'quota_exhausted'", () => {
  ...
  assert.equal(
    classify429({ status: 429, body: "Out of credits â€” top up your account." }),
    "quota_exhausted"
  );
...
test("classify429: Antigravity quota patterns do not over-match plain rate limits", () => {
  // The new 'quota reached' / 'enable overages' patterns must stay specific â€”
  ...
  // quota_exhausted â€” only the specific "individual quota reached" wording is.
  assert.equal(
...
test("classify429: 429 without quota keyword returns 'rate_limit'", () => {
  // Plain rate-limit message â€” keyword 'rate' alone is NOT in QUOTA_PATTERNS
...
test("ambiguous 'daily rate limit' messages classify as quota_exhausted (intentional)", () => {
  ...
  // because daily/monthly caps semantically warrant a long cooldown â€” even
