...
const registryFixture: Record<string, RegistryEntry> = {
  ...
  anthropic: {
    ...
    format: "claude",
    executor: "default",
    baseUrl: "http://localhost:20128/v1/messages",
    authType: "apikey",
...
test("manifest exposes API-key default-executor providers as sidecar candidates", () => {
  ...
  assert.equal(openai.endpoints.baseUrl, "http://localhost:20128/v1/chat/completions");
...
test("manifest advertises usage-supported for providers whose usage API is accepted (#10078)", () => {
  // claude is in USAGE_SUPPORTED_PROVIDERS, openai is not â€” assert against the real
...
test("usage-supported matches only on id, not alias (#10078)", () => {
  // USAGE_SUPPORTED_PROVIDERS is checked with a plain .includes(providerId) â€” no alias
  ...
  assert.equal(
    entry.capabilities.includes("usage-supported"),
    false,
    "usage-supported is id-only â€” an alias hit must not advertise it"
  );
