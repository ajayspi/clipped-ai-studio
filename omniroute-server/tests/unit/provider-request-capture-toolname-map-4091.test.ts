/**
 * #4091 â€” third-party tool-name cloak map must survive the request-capture
 ...
 * aliasâ†’original map as a NON-ENUMERABLE `_toolNameMap` on the request body.
 ...
 * `JSON.parse(JSON.stringify(...))` â€” which drops non-enumerable properties.
 ...
 * (`mergeResponseToolNameMap` â†’ `remapToolNamesInResponse`) sees an empty map
 ...
 * `mcp__â€¦` name: #4861 deliberately exempts the `mcp__` namespace from cloaking
...
test("#4091 body() preserves the non-enumerable cloak map dropped by the capture round-trip", () => {
  ...
  // The fetch-capture serializes the outgoing body and rebuilds it â€” exactly
  // what `captureFetchRequest` does â€” which strips the non-enumerable map.
...
test("#4091 body() falls back to the original body when nothing was captured", () => {
  const { capture } = makeCapture();
  const transformedBody = makeCloakedClaudeBody();
  // No capture() call â€” body() returns the fallback, which already has the map.
  const finalBody = capture.body(transformedBody) as Record<string, unknown>;
  assert.ok(finalBody._toolNameMap instanceof Map);
});
