...
// #9100 â€” proxy concurrency regression.
//
...
// serialized 5 concurrent requests behind ~30s stalls (1 fast + 4Ã— ~29.5s).
...
//      (keep-alive reuse â€” with the old 1ms TTL each queued request would have
...
test("#9100: 5 concurrent requests through a mocked HTTP proxy all resolve over ONE reused TCP connection", async () => {
  ...
  try {
    ...
    // Release the pooled keep-alive sockets BEFORE closing the listener â€”
  ...
  const results = await Promise.all(
    Array.from({ length: 5 }, (_, i) =>
      runWithProxyContext(VERCEL_CTX, () =>
        proxyFetch(
          `http://localhost:20128/v1/messages?x=${i}`,
