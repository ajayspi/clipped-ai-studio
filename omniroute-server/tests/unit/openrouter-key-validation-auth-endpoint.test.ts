// #11226 â€” OpenRouter key validation was vacuous: the probe targeted the PUBLIC
...
//   GET /api/v1/models   â†’ 200 without any auth (public catalog)
//   GET /api/v1/auth/key â†’ 401 {"error":{"message":"User not found.","code":401}} for a bad key
...
function stubRealOpenRouter() {
  ...
  globalThis.fetch = (async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (url.includes("/models")) {
      // Public catalog â€” answers 200 regardless of the Authorization header.
...
describe("openrouter registry â€” authenticated key-validation endpoint (#11226)", () => {
...
describe("omniroute providers test â€” openrouter probe (#11226)", () => {
  it("marks a bad key INVALID even though the public /models endpoint answers 200", async () => {
    const stub = stubRealOpenRouter();
    try {
