...
async function seedConnection(provider: string, overrides: any = {}) {
  return providersDb.createProviderConnection({
    ...
    // Unique per connection by default â€” real accounts have distinct keys, and
...
test("extractApiKey parses bearer headers and isValidApiKey validates persisted keys", async () => {
  ...
  // Security follow-up (#3300): query-string token fallbacks were removed â€” a
  ...
  // The path-scoped `/vscode/<token>/â€¦` form (VS Code integration) still works.
  assert.equal(
    auth.extractApiKey(
  ...
  // â€¦but never when the caller opts out of URL extraction (management auth path).
  assert.equal(
    auth.extractApiKey(
      new Request(`http://localhost/api/v1/vscode/${created.key}/chat/completions`),
      { allowUrl: false }
...
test("getProviderCredentialsWithQuotaPreflight skips the upstream fetcher when no limits are configured", async () => {
  ...
  // fetcher that throws if invoked â€” any invocation surfaces as a test
...
test("getProviderCredentialsWithQuotaPreflight invokes the fetcher when the global default is restrictive", async () => {
  // No per-connection override and no provider-window defaults â€” but the
...
test("getProviderCredentialsWithQuotaPreflight invokes the fetcher when an override IS set", async () => {
  ...
  quotaPreflight.registerQuotaFetcher("openai", async () => {
    fetcherCalls++;
    return null; // null â†’ preflight proceeds, no skip
  });
...
test("getProviderCredentialsWithQuotaPreflight: explicit quotaPreflightEnabled:false bypasses preflight even when provider has global window defaults (#2831)", async () => {
  ...
  // threshold) â€” this is the field that previously caused the gate to keep going.
  await (
...
test("getProviderCredentials refuses a forced pin outside allowedConnections instead of falling back (#12080)", async () => {
  ...
  // policy-allowed pool keeps its constraint â€” resolution yields no credential instead
...
// #3027 â€” a per-model subscription/permission 403 from a passthrough provider
...
test("markAccountUnavailable: ollama-cloud per-model subscription 403 locks the model, not the connection (#3027)", async () => {
  ...
  // Connection stays eligible â€” only the paid model is cooled down.
  assert.equal(result.shouldFallback, true);
  assert.ok(result.cooldownMs > 0);
  assert.equal(updated.testStatus, "active");
  assert.equal(updated.rateLimitedUntil, undefined);
...
// #3027 regression â€” a genuine whole-key 403 (deactivated/banned key) must NOT
...
// #3027 â€” repeated subscription 403s on the paid model must never escalate a
