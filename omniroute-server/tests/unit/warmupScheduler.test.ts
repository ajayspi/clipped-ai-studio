 ...
 *
 * Two layers:
 *   1. Pure/env helpers â€” enabled flag, cron default, concurrency clamp, PT conversion.
 *   2. Integration â€” a real temp DB with provider connections + mocked global fetch
 *      drives the full executeWarmup path: opt-in gating, classifyForWarmup,
 *      circuit-breaker skip, 401â†’refreshâ†’retry, 403 stop, 429 Retry-After parse,
...
test("startWarmupScheduler: disabled â†’ null (default)", async () => {
...
test("startWarmupScheduler: enabled â†’ returns timer and is a singleton", async () => {
  const { startWarmupScheduler, stopWarmupScheduler } =
    await import("../../src/lib/warmupScheduler.ts");
...
test("integration: opt-in gating â€” connection not in claudeWarmup.connections is skipped", async () => {
  ...
  // Do NOT opt in â€” leave claudeWarmup.connections empty.
...
test("integration: opted-in claude_pro connection â†’ fetch fires with Bearer token + beta suffix", async () => {
  const { startWarmupScheduler, stopWarmupScheduler } =
    await import("../../src/lib/warmupScheduler.ts");
  const settingsDb = await import("../../src/lib/db/settings.ts");
...
test("integration: message rotation â€” different content across sequential pings", async () => {
  const { startWarmupScheduler, stopWarmupScheduler } =
    await import("../../src/lib/warmupScheduler.ts");
  const settingsDb = await import("../../src/lib/db/settings.ts");
  ...
  // Second ping (new scheduler instance, same counter continues) â€” content should differ eventually.
  startWarmupScheduler();
  await new Promise((r) => setTimeout(r, 30));
  stopWarmupScheduler();
  mock.restore();
...
test("integration: 403 â†’ forbidden persisted, no further fetch for that connection", async () => {
  const { startWarmupScheduler, stopWarmupScheduler } =
    await import("../../src/lib/warmupScheduler.ts");
  const settingsDb = await import("../../src/lib/db/settings.ts");
...
test("integration: 429 â†’ rate_limit with Retry-After parsed", async () => {
  const { startWarmupScheduler, stopWarmupScheduler } =
    await import("../../src/lib/warmupScheduler.ts");
  const settingsDb = await import("../../src/lib/db/settings.ts");
  const crs = await import("../../src/lib/db/connectionRuntimeState.ts");
