...
test("grok-web validator: full DevTools cookie blob is parsed for the sso value", async () => {
  ...
  assert.equal(result.valid, true);
  // #5350 â€” the outbound cookie now forwards the Cloudflare cookies too.
...
test("grok-web validator: generic non-auth 403 maps to IP-reputation guidance, not 'invalid cookie' (#3474)", async () => {
  ...
  // block â€” the cookie itself is likely fine. The message must point the user to a
...
test("grok-web validator: anti-bot 'Request rejected' 403 maps to IP-reputation guidance (#3474)", async () => {
  ...
  assert.equal(result.valid, false);
  assert.match(result.error || "", /residential IP|proxy/i);
  // Cookie may be fine â€” must not claim it is invalid/expired.
...
test("grok-web validator: structured non-auth 403 (resource error) still surfaces upstream body for maintainers (#3474)", async () => {
  ...
  // by the IP-reputation guidance â€” the maintainer needs to see the real cause.
  __setGrokTlsFetchOverride(async () => {
    return {
      status: 403,
      headers: new Headers(),
...
// #5350 â€” when the user DID supply a cf_clearance, an auth-shaped 401 / invalid-credentials 403
...
test("registry providers cover remaining OpenAI-like and Claude-like validation branches", async () => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (target === "http://localhost:20128/v1/models") {
      return new Response(JSON.stringify({ data: [{ id: "gpt-4o-mini" }] }), { status: 200 });
    }
    if (target === "http://localhost:20128/v1/messages?beta=true") {
  ...
  globalThis.fetch = async (url) => {
    ...
    if (String(url) === "http://localhost:20128/v1/chat/completions") {
...
test("BytePlus key validation reaches the Ark endpoint instead of 'not supported' (#3877)", async () => {
  ...
  // registry, so validation returned {unsupported:true} â†’ UI showed "invalid" for any
...
test("specialty validator accepts Nous Research key when probe model is rejected (400)", async () => {
  ...
  // must still validate â€” the 4xx proves auth was accepted, only the request shape
...
// â”€â”€â”€ claude-web validator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
test("claude-web validator: 200 from /api/organizations â†’ valid", async () => {
...
test("claude-web validator: 401 â†’ invalid session cookie", async () => {
...
test("claude-web validator: 429 â†’ invalid (rate limited session is not healthy, #9406)", async () => {
  __setClaudeTlsFetchOverride(async () =>
...
test("claude-web validator: 500 â†’ Claude.ai unavailable", async () => {
...
test("claude-web validator: TLS client unavailable â†’ clear error", async () => {
...
// â”€â”€â”€ gemini-web validator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

test("gemini-web validator: 200 from gemini.google.com â†’ valid", async () => {
  globalThis.fetch = async (url, init = {}) => {
    const target = String(url);
    const headers = init.headers || {};
...
test("gemini-web validator: 401 â†’ invalid cookie", async () => {
  globalThis.fetch = async () => new Response("unauthorized", { status: 401 });
...
test("gemini-web validator: 500 â†’ unavailable", async () => {
...
// â”€â”€â”€ copilot-web validator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

test("copilot-web validator: valid access_token â†’ 200", async () => {
  globalThis.fetch = async (url, init = {}) => {
    const target = String(url);
...
test("copilot-web validator: 401 â†’ invalid token", async () => {
  globalThis.fetch = async () => new Response("unauthorized", { status: 401 });
...
test("copilot-web validator: 500 â†’ unavailable", async () => {
  globalThis.fetch = async () => new Response("down", { status: 500 });
...
test("copilot-web validator: empty input â†’ paste prompt", async () => {
  globalThis.fetch = async () => {
...
// â”€â”€â”€ copilot-m365-web validator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ t3-web validator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

test("t3-web validator: valid cookies â†’ valid", async () => {
  globalThis.fetch = async (url, init = {}) => {
...
test("t3-web validator: 500 â†’ unavailable", async () => {
  globalThis.fetch = async () => new Response("down", { status: 500 });
...
test("t3-web validator: valid cookies â†’ passes through", async () => {
  globalThis.fetch = async (url, init = {}) => {
    if (String(url).includes("t3.chat")) {
      return new Response("ok", { status: 200 });
    }
...
// â”€â”€â”€ Gitlawb Opengateway specialty validators â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Gitlawb-GMI (GMI Cloud) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// #3288 / #3758: a blocked redirect (REDIRECT_BLOCKED) to a PUBLIC host is benign â€” the
...
// â”€â”€â”€ huggingface validator (whoami-v2 auth probe) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
