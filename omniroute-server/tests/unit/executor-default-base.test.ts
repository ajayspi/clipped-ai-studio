...
test("DefaultExecutor.buildUrl handles Gemini and Claude variants", () => {
  ...
  assert.equal(
    ...
    "http://localhost:20128/v1/models/gemini-2.5-flash:generateContent"
  );
  assert.equal(
    gemini.buildUrl("gemini-2.5-flash", true),
    "http://localhost:20128/v1/models/gemini-2.5-flash:streamGenerateContent?alt=sse"
  );
...
test("DefaultExecutor.buildUrl honors a custom providerSpecificData.baseUrl for the built-in openai provider", () => {
  ...
  // No override â†’ hardcoded OpenAI endpoint (unchanged behavior).
  assert.equal(openai.buildUrl("gpt-4o", true), "http://localhost:20128/v1/chat/completions");
...
// `param: response_format`) â€” same class as the opencode #9992 fix, but the
...
test("DefaultExecutor.execute does not produce duplicate anthropic-version header when Claude Code CLI headers are present", async () => {
  ...
  // Must be exactly one key â€” not multiple case variants that undici would combine
