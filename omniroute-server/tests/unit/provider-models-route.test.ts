...
test("provider models route returns a static local catalog for non-LLM search/agent providers (#5569/#5571/#5573/#5575)", async () => {
  ...
  for (const { provider, expectId } of cases) {
    ...
    // RED before the fix: these had no static catalog â†’ 400 "does not support models listing".
...
test("provider models route fetches the live AI/ML API catalog from the auth-free /models endpoint (#5570)", async () => {
  ...
  // RED before the fix: aimlapi had no PROVIDER_MODELS_CONFIG entry â†’ stale
...
// default allows LAN/loopback hosts) â€” see provider-models-route-lan-guard.test.ts for the
...
test("provider models route correctly strips standard /v1 paths", async () => {
  const connection = await seedConnection("openai-compatible-standard-v1", {
    apiKey: "sk-standard",
    providerSpecificData: { autoFetchModels: true, baseUrl: "http://localhost:20128/v1" },
  });
  const seenUrls: string[] = [];
  ...
  assert.equal(response.status, 200);
  assert.equal(body.source, "api");
  assert.deepEqual(seenUrls, ["http://localhost:20128/v1/models"]);
});
...
test("provider models route strips /v1 when it precedes /chat/completions (#5899 no double /v1)", async () => {
  ...
  // "https://api.airforce/v1/chat/completions" must probe ".../v1/models" â€” NOT
  ...
  assert.equal(response.status, 200);
  assert.equal(body.source, "api");
  // First probed endpoint must have a single /v1 â€” no ".../v1/v1/models".
...
test("provider models route continues probing past a REDIRECT_BLOCKED endpoint (#5899)", async () => {
  ...
  // not abort the whole probe loop â€” discovery should fall through to the next
  ...
  globalThis.fetch = async (url) => {
    const u = String(url);
    seenUrls.push(u);
    // First candidate ".../v1/models" answers with a real 308 redirect â†’
...
test("provider models route prefers the remote OpenRouter /models API over static image models", async () => {
  ...
  // #6976 â€” OpenRouter's live /v1/models never lists embeddings/rerank (they live
  ...
  // (hasChatRegistry is true for openrouter â€” see staticModels.ts).
...
test("provider models route flags intentional local-catalog-only providers so model-sync imports them (#5460/#5465)", async () => {
  // reka + voyage-ai never do a remote /models fetch â€” their local catalog is
...
test("provider models route does NOT flag a degraded remote-fetch fallback as intentional (#5460/#5465)", async () => {
  ...
  // to the local catalog â€” that IS degraded and must NOT be flagged intentional,
...
test("provider models route uses synced models as the authoritative local catalog (#3148)", async () => {
  ...
  // provider, the route must surface the synced list â€” even on a connection
  // that never ran the sync itself â€” instead of the static catalog.
  const connection = await seedConnection("opencode-go", {
    apiKey: "opencode-go-key",
    providerSpecificData: {
  ...
  assert.equal(body.source, "local_catalog");
  assert.equal(called, false);
  // Synced models become the catalogâ€¦
  assert.ok(
    body.models.some((model) => model.id === "synced-only-model"),
    "synced model should be present in the local catalog"
  );
  // â€¦and the static catalog entries are no longer surfaced for this provider.
  assert.equal(
...
test("provider models route retries Antigravity discovery endpoints before returning remote models", async () => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    // onboardUser is a bootstrap hop too (ff012ff420) â€” else it eats the single 503 below.
    ...
    // â†’ the route falls back to local_catalog instead of returning the remote (api) list.
    return Response.json({
      models: [
  ...
  // hit â€” `:models` URLs are never reached. Assert on the actual discovery
