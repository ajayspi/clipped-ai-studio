 ...
 * classification). No live Gemini API key needed â€” the tracker counters
 * are incremented directly and a synthetic 429 error is passed to
 * checkFallbackError.
 *
 * This validates that the whole pipeline works:
 *   incrementRequestCount â†’ isRpdExhausted / isRpmExhausted â†’ checkFallbackError
 *
 * Covers three classification outcomes:
 *   - RPM exhausted â†’ RATE_LIMIT_EXCEEDED (exponential backoff)
 *   - RPD exhausted â†’ QUOTA_EXHAUSTED (midnight lockout)
 *   - Neither exhausted â†’ falls through to generic 429 (RATE_LIMIT_EXCEEDED)
 */
...
// â”€â”€ Scenario 1: RPM exhausted, RPD not exhausted â†’ RATE_LIMIT_EXCEEDED â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Scenario 2: RPD exhausted â†’ QUOTA_EXHAUSTED â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Scenario 3: Neither RPM nor RPD exhausted â†’ falls through to generic 429 â”€â”€
...
// â”€â”€ Scenario 4: Both limits exhausted â†’ RPD takes priority â†’ QUOTA_EXHAUSTED â”€â”€

test("Gemini 2.5 Flash both RPM and RPD hit: RPD check runs first â†’ QUOTA_EXHAUSTED", () => {
...
// â”€â”€ Scenario 5: 16 RPM hit (RPD=500 untouched) â†’ RATE_LIMIT_EXCEEDED â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Scenario 6: Non-Gemini provider bypasses the Gemini-specific check â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
test("Non-Gemini provider: tracker state is irrelevant, 429 goes through generic path", () => {
  ...
  // Provider is "openai" â€” Gemini-specific check is skipped
  const result = checkFallbackError(
    429,
    GEMINI_429_BODY,
    0,
  ...
  // Falls through to generic 429 handling â†’ RATE_LIMIT_EXCEEDED
  assert.equal(result.shouldFallback, true);
  assert.equal(result.reason, RateLimitReason.RATE_LIMIT_EXCEEDED);
});

// â”€â”€ Scenario 7: Reset clears state â†’ no longer exhausted â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Scenario 8: RPD exhaustion overrides RPM classification â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

test("RPD exhaustion overrides RPM classification (RPD checked first)", () => {
  // gemini-2.5-flash: RPM=5, RPD=20 â€” 25 requests exhausts both
  for (let i = 0; i < 25; i++) incrementRequestCount("gemini-2.5-flash");
  assert.equal(isRpdExhausted("gemini-2.5-flash"), true);
  assert.equal(isRpmExhausted("gemini-2.5-flash"), true);
...
// â”€â”€ Scenario 9: Unknown model (no RPM/RPD in JSON) â†’ generic 429 path â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Scenario 10: TPM exhausted, RPD not exhausted â†’ RATE_LIMIT_EXCEEDED â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Scenario 11: TPM + RPD both exhausted â†’ RPD first â†’ QUOTA_EXHAUSTED â”€â”€â”€â”€â”€â”€â”€

test("Gemini 2.5 Flash TPM and RPD both hit: RPD takes priority â†’ QUOTA_EXHAUSTED", () => {
  ...
  // RPD check runs first â†’ QUOTA_EXHAUSTED
  assert.equal(result.reason, RateLimitReason.QUOTA_EXHAUSTED);
});

// â”€â”€ Scenario 11b: real upstream TPM 429 with ZERO local counter state (#7360) â”€
//
...
test("classifyGeminiQuotaMetricFromText: real TPM error text â†’ 'tpm'", () => {
  assert.equal(classifyGeminiQuotaMetricFromText(REAL_GEMINI_TPM_429_BODY), "tpm");
});

test("classifyGeminiQuotaMetricFromText: RPD-style metric text â†’ 'rpd'", () => {
  const body =
...
test("classifyGeminiQuotaMetricFromText: RPM-style metric text â†’ 'rpm'", () => {
  const body =
    "Quota exceeded for metric: generativelanguage.googleapis.com/" +
...
test("classifyGeminiQuotaMetricFromText: no recognizable metric â†’ null", () => {
...
test("Real Gemma-4 TPM 429 body with ZERO local counter state â†’ RATE_LIMIT_EXCEEDED, not QUOTA_EXHAUSTED", () => {
  // No incrementTokenUsage/incrementRequestCount calls â€” this reproduces a
...
// â”€â”€ Scenario 12: TPM tracking and query â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Scenario 13: resetCounters clears TPM state too â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
