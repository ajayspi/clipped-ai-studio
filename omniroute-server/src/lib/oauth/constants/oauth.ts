...
// (#12417) â€” see providers/github.ts and providers/ghe-copilot.ts.
import {
...
export const CLAUDE_CONFIG = {
  ...
  tokenUrl: "http://localhost:20128/v1/oauth/token",
  redirectUri:
...
export const CODEX_CONFIG = {
  ...
  extraParams: {
    ...
    // Ported from ndycode/codex-multi-auth (auth.ts: forceNewLogin option) â€”
...
// CodeBuddy CN (Tencent â€” copilot.tencent.com) OAuth Configuration
// (Custom Device-Auth Flow: POST stateUrl â†’ open authUrl â†’ GET pollUrl?state=).
// No client_id/secret â€” the upstream CLI ships none.
...
// Grok Build (xAI) OAuth Configuration (Browser PKCE Flow â€” added #7013)
...
export const GROK_BUILD_OAUTH_CONFIG = {
  ...
  codeChallengeMethod: "S256",
  loopbackPort: 56122, // distinct from xai-oauth's 56121 â€” both can run concurrently
...
export const ANTIGRAVITY_CONFIG = {
  ...
  // No "openid" scope â€” the working 9router flow requests only the Cloud Code /
...
// public credentials and Code Assist endpoints â€” no new embedded secret â€” and the same
...
// Re-uses CODEX_CONFIG.clientId to avoid duplication â€” same provider, different originator.
// IMPORTANT: same Auth0 backend as Codex â†’ same multi-account session-takeover
...
// AWS region allowlist â€” prevents SSRF via region injection into upstream URLs
...
export const KIRO_CONFIG = {
  ...
  // `socialClientId` is a public CLI identifier â€” Kiro's device endpoint accepts
...
// clientVersion pin lives in open-sse/utils/cursorAgentCliVersion.ts â€” single source of truth.
export const CURSOR_CONFIG = {
  ...
  // Client metadata â€” pin from cursorAgentCliVersion (not a second hardcoded string)
...
// token only â€” users sign in inside Trae and paste the resulting API token
...
export const TRAE_CONFIG = {
  ...
  // Trae website â€” users retrieve their token here after signing in
  webUrl: "https://trae.ai",
  // SOLO remote agent base â€” the executor's real upstream. Also set as the
  ...
  // Observed Cloud-IDE-JWT lifetime â€” drives default expiry hints.
  tokenLifetimeDays: 14,
  // Token storage note â€” solo.trae.ai exposes no public SQLite/keychain path,
...
// Zed IDE credential import â€” no standard OAuth flow.
...
// the token with the matching private key â€” see open-sse/shared/zedAuth.ts.
// No client_id/secret/Firebase key is embedded here (Hard Rule #11 does not
// apply â€” there is no upstream secret to embed).
