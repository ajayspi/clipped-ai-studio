...
// (MiniMax, Z.AI, â€¦) do NOT â€” they reject remote URLs (MiniMax: 403 code
 ...
 *   2. `VISION_BRIDGE_API_KEY` env var â€” operator-set, takes precedence over
...
export function resolveProviderApiKey(model: string, explicitKey?: string): string {
  ...
  // VISION_BRIDGE_API_KEY only applies to the OpenAI-compatible branch â€” the
 ...
 *   1. VISION_BRIDGE_API_KEY env (already handled by resolveProviderApiKey â€”
 *      kept here for the injected-resolver test path).
 *   2. Injected resolver (tests) or the DB-backed `getOrCreateApiKey()` â€”
 ...
 *
 * Priority:
 *   1. `VISION_BRIDGE_BASE_URL` env var â€” operator-set, e.g. point this at
 ...
 *   3. OmniRoute self-loop (`http://localhost:20128/v1`) â€” auto-detected when
 *      the model uses a known OmniRoute-internal provider (e.g. `kr/`, `if/`,
 *      `pol/`, `groq/`, etc.) instead of a direct OpenAI/Anthropic endpoint.
 *   4. `http://localhost:20128/v1` (fallback when the model is `openai/*` or
 *      unprefixed â€” works only when the operator actually has an OpenAI
 ...
 * `open-sse/executors/commandCode.ts::extractImageUrl` â€” without it, a
 ...
 * combo-filter material only â€” extracting it would desync the positional
...
export function extractImageParts(messages: RequestMessage[]): ImagePart[] {
  ...
  // replaceImageParts can splice back â€” the extractâ†”replace contract: every
...
export async function ensureBase64ImagesForClaudeWire(
  ...
  // Map sequential image index â†’ resolved data URI (null = keep original).
...
async function fetchRemoteImageAsDataUri(
  ...
  const remoteImage = await fetchRemoteImage(imageUrl, {
    signal,
    // Bypass the runtime's hooked global fetch (ProxyFetch) â€” a dead local
  ...
  // model self-call â€” scoped to this bridge-fetched image only, never the
...
export function composeVisionPrompt(
  ...
  const hint = text.length > 500 ? `${text.slice(0, 500)}â€¦` : text;
...
export async function callVisionModel(
  ...
  // injectable credential-check seam â€” without forwarding it, tests (and any
...
function parseSseVisionBody(rawBody: string): unknown {
  ...
  for (const line of trimmed.split(/\r?\n/)) {
    ...
    // Error-only SSE chunk (`data: {"error":{...}}` with no choices) â€” surface
...
async function readVisionResponseBody(response: Response): Promise<unknown> {
  try {
    // JSON path â€” also unwrap the { _streamed, summary } diagnostics envelope
    // that some OmniRoute capture paths attach to provider responses.
    return unwrapVisionSummary(await response.json());
  } catch {
    // Not JSON â€” attempt SSE / envelope parsing from the raw text.
  }
 ...
 * Falls back to `reasoning_content` then `reasoning` when `content` is empty â€”
...
async function callVisionModelSingle(
  ...
  // â€¦) cannot ingest remote image URLs â€” normalize to base64 so the self-loop
  // body reaches the backend as a data URI (the OpenAIâ†’claude translator only
  ...
  try {
    ...
    if (isAnthropic) {
      ...
      // OpenAI-compatible path (default) â€” issue #2232: honor
              ...
              content: [
                {
                  ...
                  // caller/provider when the target model lacks vision support â€”
                  ...
                  // â€” low-detail sampling degrades OCR accuracy for every
    ...
    if (isAnthropic) {
      ...
      // content is null â€” reasoning models (e.g. xiaomi/mimo-v2.5) can exhaust
...
export function replaceImageParts(
  body: RequestBody,
  // #4012: a `null` entry means the describe call failed for that image â€” keep
  ...
  for (let msgIdx = 0; msgIdx < requestMessages.length; msgIdx++) {
    ...
    for (const part of message.content) {
      ...
      // replaceable â€” extractImageParts allowlists it, and every extracted
      // part needs a matching splice here (extractâ†”replace contract).
      ...
      if (partType === "image_url" || partType === "image" || partType === "input_image") {
        if (descriptionIndex < descriptions.length) {
          ...
          descriptionIndex++;
          if (description == null) {
            // #4012: describe failed for this image â€” preserve the original
