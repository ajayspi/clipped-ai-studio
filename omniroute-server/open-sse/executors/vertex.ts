...
//     catalog from generativelanguage.googleapis.com/v1beta/models â€” without it that listing returns
 ...
 * support BOTH: Service Account JSON (JWT â†’ OAuth â†’ project-scoped endpoint + Bearer auth) and
...
const PARTNER_MODELS = new Set([
  ...
  // silently break every time Anthropic ships a new generation â€” see issue #1985.
  "claude-",
  "deepseek-v3",
  "deepseek-v3.2",
  "deepseek-v4",
...
// other PARTNER_MODELS entries (DeepSeek, Qwen, Llama, Mistral, GLM) go through â€” the OpenAI-shaped
...
// â€” which already parse real message_start/content_block_*/message_delta/message_stop events â€”
// can consume it correctly, instead of relying on the OpenAI-`choices`-only JSONâ†’SSE fallback
...
export class VertexExecutor extends BaseExecutor {
  ...
  async execute(input: ExecuteInput) {
    ...
    // Service Account JSON â†’ mint a short-lived OAuth token (Bearer). An Express-mode API key is
    ...
    if (isClaudeModel(model) && input.body && typeof input.body === "object") {
      ...
      // one (the openaiâ†’claude request translator copies the client's model field over).
      delete body.model;
    }
  ...
  buildUrl(model: string, stream: boolean, urlIndex = 0, credentials: any = null) {
    ...
    // ?key= query parameter (verified working contract â€” same as the CaptionAI GeminiClient). The
