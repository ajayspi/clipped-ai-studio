...
function applyZaiGlm53OpenAIDefaults<T>(
  ...
  if (record.reasoning_effort === undefined && record.reasoning === undefined) {
    ...
    // request with no effort â€” Pi "off", plain API calls â€” maps to the floor
    // tier "low" per the declared-tier clamp convention (none/minimal â†’ low),
...
export class DefaultExecutor extends BaseExecutor {
  ...
  buildUrl(model, stream, urlIndex = 0, credentials = null) {
    ...
    if (this.provider?.startsWith?.("openai-compatible-")) {
      ...
      const baseUrl = psd?.baseUrl || "http://localhost:20128/v1";
    ...
    // URL, so it must bypass the per-provider normalizers in the switch below â€”
    ...
    // e.g. appending "/chat/completions" to an Anthropic ".../v1/messages" endpoint.
    {
      ...
      if (alternate?.baseUrl && !hasManualBaseUrl) {
        ...
        // builds its own URL â€” chatPath/urlSuffix are constants and cannot carry the model.
    ...
    switch (this.provider) {
      ...
      case "openai": {
        ...
        // the native /responses endpoint â€” the per-model targetFormat (registry tag
      ...
      case "poe": {
        // #8969: Poe API-key surfaces â€” Chat Completions, Responses, and
        // Claude-only Messages. Prefer the responses marker from
        // resolveExecutionCredentials (incoming /v1/responses), then the
        // registry Claude targetFormat â†’ messagesUrl, else chat/completions.
      ...
      default: {
        ...
        // (http://localhost:20128/v1/...). Scoped to openai-format providers so
    ...
    switch (this.provider) {
      ...
      case "anthropic":
        if (effectiveKey) {
          ...
          // Authorization: Bearer ALONGSIDE x-api-key â€” without it they
        ...
        // auth header â€” the handler will produce a clean "no credentials"
      ...
      case "glm-coding-apikey":
        ...
        break;
      case "clinepass": // dual-auth (OAuth or BYOK) â€” see applyClineAuthHeaders()
   ...
   * verified live 2026-08-15 â€” same class as #9992's opencode fix). Gated so
  ...
  applyJsonSchemaFallback<T>(body: T): T {
    ...
    // openai-compatible-* providers accept json_object natively â€” only the
    ...
    // both json_schema AND json_object 400 with `param: response_format`) â€” strip
   ...
   * "org/model-name") â€” we must NOT strip path segments. (Fix #493)
   */
  transformRequest(model, body, stream, credentials) {
    ...
    // during combo routing or format translation. Build a tool_call_id â†’ function.name
    ...
    // doesn't cover it. Strip it from every message before sending â€” scoped to
    ...
    // (e.g. claude-opus-4 deprecated `temperature` â†’ Anthropic 400). Port from
  ...
  async execute(input: ExecuteInput) {
    // #6846 Phase 1: per-connection concurrency cap for nvidia â€” no-op for every
