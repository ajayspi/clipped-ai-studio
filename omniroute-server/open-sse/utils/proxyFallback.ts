/**
 * proxyFallback.ts â€” Smart Proxy Fallback for Provider Validation
 *
...
// probe the whole proxy pool simultaneously â€” a thundering herd of TCP connects
...
export async function findWorkingProxy(
  ...
  if (cached) {
    if (cached.expiresAt > Date.now()) {
      // Cached hit â€” return the proxy (or null if previously all failed)
      return cached.proxyUrl || null;
    }
    // Expired entry â€” remove it and re-probe
    PROXY_FALLBACK_CACHE.delete(cacheKey);
  }

  // #9100: single-flight â€” if a probe for this cache key is already running,
  ...
  const probe = (async (): Promise<string | null> => {
    ...
    // All failed â€” cache the negative result to avoid re-probing too often
    PROXY_FALLBACK_CACHE.set(cacheKey, {
      proxyUrl: "",
      expiresAt: Date.now() + CACHE_TTL_MS,
    });
  ...
  try {
    ...
    // Only the owning caller removes the entry â€” a later caller that picked up
...
export async function selectWorkingProxyFallback(_connectionId?: string): Promise<{
  ...
  // assignments / per-connection proxy_enabled). Default OFF â€” only run when the
  ...
  const targetUrl = "http://localhost:20128/v1/models";
