...
export function detectFormatFromEndpoint(body, endpointPath = "") {
  ...
  // (mirrors /messages â†’ claude) makes the pipeline translate the request
  // antigravityâ†’openai and the response openaiâ†’antigravity, so the IDE gets
...
// path chatCore already threads) â€” single source of truth stays detectFormatFromEndpoint.
...
// - If lastMessage is not user â†’ remove Claude/Gemini-style thinking config
