/**
 * usage/claude.ts â€” Claude (Anthropic OAuth + legacy org) usage fetcher + plan-label helper.
 *
 * Extracted from services/usage.ts (god-file decomposition): the Claude family â€” the API
 ...
 * leaves + Claude identity/cooldown helpers + safePercentage â€” no host coupling â€” so it
...
const CLAUDE_CONFIG = {
  ...
  usageUrl: "http://localhost:20128/v1/organizations/{org_id}/usage",
  settingsUrl: "http://localhost:20128/v1/settings",
...
export async function getClaudeUsage(accessToken?: string) {
  ...
  // (chat with the same token still works â€” only the quota endpoint is throttled).
  ...
  try {
    // Real CLI uses axios here, not Stainless â€” UA is `claude-code/<version>`
    ...
    if (oauthResponse.ok) {
      ...
      // Map Anthropic's internal codenames (e.g., omelette â†’ Designer) for display.
    ...
    // Cool down OAuth usage polling after a 429 (quota endpoint only â€” chat is unaffected).
