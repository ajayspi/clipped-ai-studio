...
// successful completion (truncated at the token limit, or a tool-call turn) â€”
...
export function isEmptyContentResponse(responseBody: unknown): boolean {
  ...
  if (Array.isArray(body.choices)) {
    ...
    // successful completion even with empty text â€” do not flag it as a fake success.
    const finishReason =
...
export const PROVIDER_ERROR_TYPES = {
  ...
  // fixable by entering a Project ID â€” never a model lockout and never a ban.
...
// `model "gpt-9" is not supported` â€” free-tier/aggregator providers name the
...
// cannot help â€” but routing egress through a proxy in a supported region can.
...
// refusal (GEO_BLOCK_SIGNALS above): Cloud Code / Gemini Code Assist â€” the
// antigravity executor (antigravity, agy) â€” and the Gemini Developer API
...
// Cloudflare 1010 "Access denied ... blocked based on your browser's signature" â€”
...
//
// IMPORTANT: the bare number 1010 is NOT matched on its own â€” a 403 body can
...
export function classifyProviderError(
  ...
  // 404 â€” model or endpoint not found. Without classification the error
  ...
  if (statusCode === 401) {
    ...
    // chatCore.ts â€” auto-combo keeps re-selecting the same broken model on
  ...
  // (Cloud Code / Gemini Code Assist + Gemini Developer API â€” see
  ...
  // â€” but the connection must be cached as excluded so routing does not
  ...
  if (statusCode === 403 && isCloudflareFingerprintRejection(bodyStr)) {
    ...
    // account's credentials. It says nothing about account health â€” a different client on
  ...
  if (statusCode === 403) {
    ...
    // RECOVERABLE project-config issue â€” the Cloud AI Companion API not enabled
    // on the project ("has not been used in project â€¦", SERVICE_DISABLED,
    // accessNotConfigured), a stale/mismatched project, or PERMISSION_DENIED on
    // the project â€” NOT an account ban. Real account bans are already caught by
    // isAccountDeactivated above (â†’ ACCOUNT_DEACTIVATED). Classifying these as
    ...
    // Kiro IDC missing profileArn â€” AWS returns 403 "User is not authorized to make this call"
    ...
    // Must be checked BEFORE the generic apikey-403â†’null return below, which
    ...
    // No-credential ("authType: none") providers â€” free, stateless per-request
    // token proxies â€” have no real account/credential
  ...
  // dashboard â€” classified separately so chatCore rotates to sibling accounts
