...
// SAME provider (e.g. `antigravity/gemini-3.1-flash-image-preview` â†’
// antigravity's callable `gemini-3.1-flash-image`). A cross-provider bare alias
// must NOT override an explicit prefix â€” #9982 removed the unconditional bare
...
// Kept out of getImageModelEntry() (which sits at the complexity-ratchet cap) â€” an
...
export const IMAGE_PROVIDERS: Record<string, ImageProviderConfig> = {
  ...
  openai: {
    id: "openai",
    baseUrl: "http://localhost:20128/v1/images/generations",
    authType: "apikey",
    authHeader: "bearer",
  ...
  // Requires the `agent` binary (CURSOR_AGENT_BIN) â€” see cursorAgentImage handler.
  cursor: {
    id: "cursor",
    alias: "cu",
  ...
  // OpenAI-compatible) â€” image generation lives on ai.api.nvidia.com/v1/genai/<model>
  ...
  // SenseNova (å•†æ±¤æ—¥æ—¥æ–°) Text-to-Image on the free Token Plan. OpenAI-compatible
  // `/v1/images/generations`, so the generic OpenAI image handler routes it â€” same
  ...
  // STT/TTS entries in audioRegistry.ts. Model list is deliberately small â€”
  ...
  // Not listed in the chat registry â€” image catalog only. Generation path still
  ...
  // Adobe Firefly (unofficial) â€” IMS access_token (clio-playground-web) or browser
...
export function parseImageModel(modelStr) {
  ...
  // No provider prefix â€” try to find the model in every provider, excluding cookie-auth (web) bridges
  for (const [providerId, config] of Object.entries(IMAGE_PROVIDERS)) {
    if (
...
/**
 * #6457 â€” precise provider+modelId membership check against the image registry.
 ...
 * as an image model under this exact `providerId`?" â€” used by the chat catalog
