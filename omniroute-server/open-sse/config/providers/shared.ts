/**
 * Provider Registry â€” Single source of truth for all provider configuration.
 *
...
export interface RegistryModel {
  ...
  /** Per-model upstream header-response timeout override â€” precedes
   ...
   * (`gpt-5.6` â†’ `gpt-5.6-sol`) and cross-provider spellings of the same model
   * (cursor's `claude-4.6-opus-high` â†’ `claude-opus-4-6`). Plain effort/`-free`
...
export interface RegistryEntry {
   ...
   *  provider â€” see registry/github/index.ts. */
  messagesUrl?: string;
  urlSuffix?: string;
   ...
   * providers) isn't one of the few models statically listed here at all â€”
   ...
   * text-completion backends" â€” no tool calling on any model, not just the
   ...
   * string. Used by the Responsesâ†’Chat translator to collapse single-part text
...
export const CHAT_OPENAI_COMPAT_MODELS: Record<string, RegistryModel[]> = {
  ...
  // Sweep 2026-06-19: codestral-2405 retired 2025-06-16 (Mistral official docs) â€” dropped
