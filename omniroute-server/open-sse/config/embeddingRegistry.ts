...
export interface EmbeddingModel {
   ...
   * that require a mandatory parameter â€” e.g. NVIDIA NIM `nv-embedqa-*` models
...
export interface EmbeddingProvider {
  ...
  /**
   * Set when the endpoint embeds exactly ONE text per request (`{"text": â€¦}` â†’
 ...
 * Only used for local providers (localhost) â€” caller must filter by hostname.
 */
...
export const EMBEDDING_PROVIDERS: Record<string, EmbeddingProvider> = {
  ...
  openai: {
    id: "openai",
    baseUrl: "http://localhost:20128/v1/embeddings",
    authType: "apikey",
    authHeader: "bearer",
    models: [
  ...
  // #6976 â€” OpenRouter serves embeddings via a dedicated OpenAI-compatible
  ...
  gemini: {
    ...
    baseUrl: "http://localhost:20128/v1/openai/embeddings",
    authType: "apikey",
    authHeader: "bearer",
    models: [
      {
  ...
  // Naver CLOVA Studio â€” embedding v2. The endpoint takes a single `{"text": â€¦}`
  // body and returns `{status, result:{embedding:[â€¦1024 floats], inputTokens}}`,
  ...
  // LM Studio â€” local OpenAI-compatible server. No auth required.
  ...
  // Ollama Local â€” OpenAI-compatible embeddings endpoint. Ollama exposes its
  ...
  // Issue #6660: Mixedbread AI â€” OpenAI-compatible /v1/embeddings, free tier
...
const EMBEDDING_MODEL_ALIASES: Record<string, string> = {
  ...
  // OpenRouter-style google/ prefix still resolve to the Gemini provider â€”
 ...
 * null â€” they need bespoke URL handling, not a bogus endpoint.
 *
...
export function parseEmbeddingModel(
  ...
  // No provider prefix â€” search hardcoded providers for the model
 ...
 * providers) â€” callers treat undefined as "can't assert", not "zero".
 */
