...
// completion bookkeeping to land before persisting a 499. See #9653 â€” a client
...
export const OAUTH_ENDPOINTS = {
  ...
  anthropic: {
    token: "http://localhost:20128/v1/oauth/token",
    auth: "http://localhost:20128/v1/oauth/authorize",
  },
  qoder: {
...
/**
 * #10360 â€” stable error code for an INTERNAL violation of the executor
...
// Configurable backoff steps for rate limits (Phase 1 â€” enhanced rate limiting)
...
// 1min â†’ 2min â†’ 5min â†’ 10min â†’ 20min
...
// â”€â”€â”€ Provider Resilience Profiles â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export const PROVIDER_PROFILES = {
  oauth: {
    transientCooldown: 5000, // 5s (session tokens â€” short recovery)
  ...
  // Not yet wired into getProviderProfile() â€” will be used when local provider_nodes
  // are integrated into the resilience layer. Kept here to avoid a second constants change.
  local: {
    transientCooldown: 2000, // 2s (local â€” very fast recovery)
    rateLimitCooldown: 5000, // 5s (local â€” no real rate limits)
...
// These are intentionally HIGH â€” they won't restrict normal usage.
// Real limits are learned from provider response headers.
export const DEFAULT_API_LIMITS = {
  requestsPerMinute: 60, // 60 RPM (reduced from 100 â€” saves Bottleneck queue memory)
...
// â”€â”€ Credential Health Check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
 ...
 * - BUFFER_MAX_BYTES: hard cap on the held window â€” commit (flush + passthrough)
...
export const STREAM_RECOVERY = {
   ...
   * outcome â€” there was nothing to de-duplicate) is indistinguishable, from string data
   ...
   * of legitimate low-overlap continuations (bounded retry, then a clean close â€” no data
