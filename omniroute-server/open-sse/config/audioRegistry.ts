...
export interface AudioProvider {
   ...
   * but their connections are stored under the node **id** â€” without this the
...
export const AUDIO_TRANSCRIPTION_PROVIDERS: Record<string, AudioProvider> = {
  ...
  openai: {
    id: "openai",
    baseUrl: "http://localhost:20128/v1/audio/transcriptions",
    authType: "apikey",
    authHeader: "bearer",
    models: [
  ...
  gladia: {
    id: "gladia",
    // POST https://api.gladia.io/v2/pre-recorded â€” async workflow: upload â†’ submit â†’ poll
  ...
  speechmatics: {
    id: "speechmatics",
    // POST https://asr.api.speechmatics.com/v2/jobs â€” async batch workflow:
    // submit multipart job (audio + JSON config) â†’ poll â†’ fetch transcript.
    // Auth: Authorization: Bearer <api-key>
    // Free tier: 8 hours/month, no credit card required.
    // Streaming (WebSocket real-time) mode is out of scope for v1 â€” batch only.
 ...
 * `language` input â€” output is always English regardless of source audio.
 */
export const AUDIO_TRANSLATION_PROVIDERS: Record<string, AudioProvider> = {
  openai: {
    id: "openai",
    baseUrl: "http://localhost:20128/v1/audio/translations",
    authType: "apikey",
    authHeader: "bearer",
...
export const AUDIO_SPEECH_PROVIDERS: Record<string, AudioProvider> = {
  google: {
    ...
    baseUrl: "http://localhost:20128/v1/models",
    authType: "apikey",
  ...
  openai: {
    id: "openai",
    baseUrl: "http://localhost:20128/v1/audio/speech",
    authType: "apikey",
    authHeader: "bearer",
    models: [
  ...
  // â”€â”€ Cloud TTS Providers (#248) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  gtts: {
    id: "gtts",
    // Google Translate TTS â€” reverse-engineered, no API key required.
    // POST batchexecute RPC (unlike the deprecated GET /translate_tts) â€”
...
export interface ProviderNodeRow {
  /** provider_node row id â€” the key its connections (and credentials) are stored under. */
  id?: string;
  prefix: string;
  name: string;
  baseUrl: string;
 ...
 *
 * Loopback nodes keep `authType: "none"` â€” a local Ollama/LM Studio has no key and
