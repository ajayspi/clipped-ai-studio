 ...
 * sources â€” base64 AND fetched URLs.
 *
 ...
 * before this handler ever runs â€” URL-sourced items are excluded from that
 ...
 * memory at once â€” 16x past the documented per-request bound. Processing
...
function geminiNativeUrl(model: string, method: "embedContent" | "batchEmbedContents"): string {
  return `http://localhost:20128/v1/models/${encodeURIComponent(model)}:${method}`;
}
 ...
 *  "result":{"embedding":[â€¦1024 floats],"inputTokens":4}}
 * ```
 *
 * There is no `data[]` and no `usage` object, so both are synthesized. `index` is
 * left at 0 here â€” the batching loop in `embeddings.ts` rewrites it to the
