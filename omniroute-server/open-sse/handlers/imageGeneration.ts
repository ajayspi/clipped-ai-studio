...
// example request JSON published on docs.kie.ai (never inferred by pattern â€”
...
//   - seedream: 5.0-lite-* drops the ".0" â€” real id is `5-lite-*`
...
//     enum uses a dash â€” real id is `wan/2-7-*`
...
//     version-specific id or "version" input field found â€” unclear whether
...
// but has no `docs.kie.ai/market/flux2/kontext` (or similar) Market page â€”
...
// NOT in KIE_MARKET_UPSTREAM_MODEL_IDS below on purpose â€” handleKieImageGeneration
 ...
 *
 * Resolution order: providerSpecificData.baseUrl â†’ credentials.baseUrl â†’ fallback.
 *
...
export function resolveImageBaseUrl(
  ...
  // polynomial-ReDoS pattern on long runs of "/" â€” CodeQL js/polynomial-redos).
...
// #8307 â€” some ChatGPT accounts can run Codex but lack entitlement for the specific
...
export async function handleImageGeneration({
  ...
  if (!providerConfig) {
    ...
    if (log) {
      log.info("IMAGE", `Custom model ${provider}/${model} â€” using OpenAI-compatible handler`);
    }

    const syntheticConfig = {
      id: provider,
      // #3205: custom OpenAI-compatible nodes store their base URL in
      // credentials.providerSpecificData.baseUrl (same as the chat path â€”
      ...
      baseUrl: resolveImageBaseUrl(
        credentials,
        `http://localhost:20128/v1/openai/images/generations`
      ),
      authType: "apikey",
      authHeader: "bearer",
      format: "openai",
...
async function handleKieImageGeneration({
  ...
  // #11296 â€” flux/kontext is catalogued with `isMarket: true`, but KIE does not
...
async function handleOpenAIImageGeneration({
  ...
  // #8085 â€” keyless Pollinations image requests (the common free case) get
...
export async function handleOpenAIImageEdit({
  ...
  const url = resolveImageBaseUrl(
    credentials,
    `http://localhost:20128/v1/openai/images/edits`,
    "edits"
  );
  ...
  // whose `FormData` class differs from `globalThis.FormData` â€” passing a native FormData
...
/**
 * Codex image generation â€” translate GPT-Image-style /v1/images/generations
...
async function handleCodexImageGeneration({
  ...
  if (!token) {
    return saveImageErrorResult({
      ...
      status: 401,
      startTime,
      error: "Codex credentials missing accessToken â€” reconnect the Codex provider",
      path: logPath,
    });
  }
  ...
  // Codex backend's defaults â€” today that's `gpt-image-2`.
    ...
    if (items.length === 0) {
      return {
        ...
        error: {
          ...
          startTime,
          error:
            "Codex completed without producing an image_generation_call â€” the model may have declined the tool",
          requestBody: requestBodyForLog,
          path: logPath,
        },
      };
...
export function saveImageErrorResult({
  ...
  requestBody = null,
  path = "/v1/images/generations",
  // #8307: opt-in signal for executeImageWithCredentialFallback â€” set by a
